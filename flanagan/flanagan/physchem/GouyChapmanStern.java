...
*   potential profiles and ionic profiles at an3
...
public class GouyChapmanStern{
    ...
    private Vector<Object> vec = new Vector<Object>();
                                            ...
                                            //  etc
    private boolean unpackVector = false;   // = true when the above vector has been unpacked for the calculations
    ...
    public void setIon(String ion, double concn, double radius, int charge, double assocK){
        ...
        this.vec.addElement(new Double(concn));
    ...
    private void unpack(){
        ...
        for(int i=0; i<numOfIons; i++){
            this.ionNames[i]    = (String)this.vec.elementAt(0+i*5);
            hold                = (Double)this.vec.elementAt(1+i*5);
            this.initConcnM[i]  = hold.doubleValue();
            this.initConcn[i]   = this.initConcnM[i]*1e3;
            hold                = (Double)this.vec.elementAt(2+i*5);
            this.radii[i]       = hold.doubleValue();
            holi                = (Integer)this.vec.elementAt(3+i*5);
            this.charges[i]     = holi.intValue();
            hold                = (Double)this.vec.elementAt(4+i*5);
   ...
   // Calculate the surface charge for the set surface potential
    ...
    public double[] getInitConcns(){
        ...
        double[] conc = (double[])this.initConcn.clone();
    ...
    public double[] getBulkConcns(){
        ...
        double[] conc = (double[])this.bulkConcn.clone();
        for(int i=0; i<this.numOfIons; i++)conc[i] *= 1e-3;
        return conc;
    }
