...
*
*  April 2012, modifying for H prediction
*
*/
public class IBEXWindH {

	public int mcN = 10000;  // number of iterations per 3D integral
	public String outFileName = "lo_H_fit_09.txt";
	...
	public double[] minSpeedsH, maxSpeedsH;

	// Interstellar mode energies
	// { spring min, spring max, fall min, fall max }
	public double[] hEnergies = { 73.7, 194.3, 4.4, 11.6 };
	...
	*/
	public IBEXWind2() {
		...
		hSpeeds = new double[hEnergies.length];

		o("calculating speed range");
		for (int i=0; i<4; i++) {

			hSpeeds[i]=Math.sqrt(2.0*heEnergies[i]/4.0/MP*EV);
			System.out.println(hEnergies[i] +" = " + hSpeeds[i] );

		}

		minSpeedsH = new double[8];
		maxSpeedsH = new double[8];

		for (int i=0; i<8; i++) {
			minSpeedsH[i]=Math.sqrt(2.0*minEnergies[i]/4.0/MP*EV);
			maxSpeedsH[i]=Math.sqrt(2.0*maxEnergies[i]/4.0/MP*EV);
			//System.out.println(minSpeedsHe[i] +" = " + maxSpeedsHe[i] + " , " + minSpeedsO[i] + " = " + maxSpeedsO[i]);
		}

		System.out.println(minSpeedsH[0] + "\t" + maxSpeedsH[1] +"\t" + hSpeeds[2] + "\t" + hSpeeds[3]);
